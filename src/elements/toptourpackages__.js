...
export default function ElementGrid(props) {
    ...
    var Class = '';
    if (props.tour.ctmClass == 1 || props.tour.ctmClass == 6 || props.tour.ctmClass == 7 || props.tour.ctmClass == 12) { Class = 'double_size' };
    ...
    return (
		<div className="element grid classic4_cols animated4">
			<div className="one_fourth gallery4 classic static filterable portfolio_type themeborder">
				<Link className="tour_image" to={"/tours/" + props.tour.id}>
					<img  src={props.tour.featuredImageUrl} alt="Niko Trip" />
					<div className="tour_price ">{ props.tour.cost_price ? 'Rs.'+props.tour.cost_price : ''}</div>
				</Link>
				<div className="portfolio_info_wrapper">

					<Link className="tour_link" to="/tours/single-tour">
						<h4>{props.tour.name}</h4>
					</Link>
					<div className="tour_excerpt">
						<p>{props.tour.tour_excerpt ? props.tour.tour_excerpt : 'Mountain, Rural, Urban'} </p>
					</div>
